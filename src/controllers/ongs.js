...
module.exports = {
  ...
  async index(req,res){
    try{
      const ongs = await connection(table).select().column('nome','email','telefone','cidade','uf');
  ...
  async show(req,res){
    let {id} = req.params;
    try{
      const ong = await connection(table).select().column('nome','email','telefone','cidade','uf').where('id',id);
